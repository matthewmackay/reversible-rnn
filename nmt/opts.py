...
def model_opts(parser):
    ...
    group = parser.add_argument_group('Reversibility')
    group.add_argument('-use_reverse', action='store_true', default=False,
                       help='Flag to use reversibility.')
    ...
    # IMPORTANT: I changed this so that input_feed is 0 (off) by default.
    group.add_argument('-input_feed', type=int, default=0,
                       help="""Feed the context vector at each time step as
                       additional input (via concatenation with the word
                       embeddings) to the decoder.""")

    group.add_argument('-rnn_type', type=str, default='LSTM',
                       choices=['LSTM', 'GRU', 'RevGRU'],
                       help="""The gate type to use in the RNNs""")
    group.add_argument('-encoder_rnn_type', type=str, default='lstm',
                       choices=['LSTM', 'GRU', 'lstm', 'gru', 'revgru', 'revlstm'],
                       help="""The gate type to use in the encoder""")
    group.add_argument('-decoder_rnn_type', type=str, default='lstm',
                       choices=['LSTM', 'GRU', 'lstm', 'gru', 'revgru', 'revlstm'],
                       help="""The gate type to use in the decoder""")
    group.add_argument('-encoder_model', type=str, default='CUDNN',
                       choices=['CUDNN', 'Vanilla', 'Rev'],
                       help="""The RNN implementation to use for the encoder""")
    group.add_argument('-decoder_model', type=str, default='CUDNN',
                       choices=['CUDNN', 'Vanilla', 'Rev'],
                       help="""The RNN implementation to use for the decoder""")
    # group.add_argument('-residual',   action="store_true",
    #                     help="Add residual connections between RNN layers.")

    group.add_argument('-enc_max_forget', type=float, default=0.875,
                       help='Encoder max no. of bits forgotten per hidden unit per time step')
    group.add_argument('-dec_max_forget', type=float, default=0.875,
                       help='Decoder max no. of bits forgotten per hidden unit per time step')

    # Dropout options (copied from AWD-LSTM)
    parser.add_argument('-dropout', type=float, default=0,
                        help='dropout applied to layers fed into classifier (0 = no dropout)')
    group.add_argument('-dropouth', type=float, default=0,
                       help='dropout for rnn layers (0 = no dropout)')
    group.add_argument('-dropouti', type=float, default=0,
                       help='dropout for input embedding layers (0 = no dropout)')
    group.add_argument('-dropoute', type=float, default=0,
                       help='dropout to remove words from embedding layer (0 = no dropout)')
    group.add_argument('-dropouto', type=float, default=0,
                       help='Dropout on the RNN output.')
    group.add_argument('-dropouts', type=float, default=0,
                       help='Dropout on the RNN output.')
    group.add_argument('-wdrop', type=float, default=0,
                       help='amount of weight dropout to apply to the RNN hidden to hidden matrix')

    group.add_argument('-context_type', type=str, default='hidden',
                       choices=['hidden', 'emb', 'slice', 'slice_emb', 'linear'],
                       help='Determines the type of context produced by the encoder (for the decoder attention)')
    group.add_argument('-slice_dim', type=int, default=0,
                       help='The dimension of the hidden slice.')
    group.add_argument('-slice_loc', type=str, default='start', choices=['start', 'end', 'both'],
                       help='The number of hidden state dimensions to use in the context slices.')


    group.add_argument('-use_buffers', action='store_true', default=False,
                       help='whether or not to use buffers (needed for exact reversibility)')
    group.add_argument('-separate_buffers', action='store_true', default=False,
                       help='whether or not to use separate rather than shared buffers for the encoder and decoder')
    ...
    group.add_argument('-global_attention', type=str, default='general',
                       choices=['none', 'dot', 'general', 'mlp', 'mlp-conc'],
...
def train_opts(parser):
    ...
    group.add_argument('-save_dir', default='saves',
    ...
    group.add_argument('-adam_beta2', type=float, default=0.999,
                       ...
                       baselines.""")
    group.add_argument('-label_smoothing', type=float, default=0.0,  # IMPORTANT: label_smoothing is 0 by default, and we don't change it
    ...
    group.add_argument('-no_log_during_epoch', type=int, default=0,
                       help="Do NOT log train and val perplexities at iterations in the middle of an epoch.")
    group.add_argument('-log_mem', action='store_true', default=False,
                       help="Log the memory usage.")
